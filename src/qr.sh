_qr_create() {
	...
	local link="https://${magic}.github.io/tools/qr.html#${vmess}"
	echo
	echo "---------- V2Ray 二维码 -------------"
	echo
...
_ss_qr() {
	local ss_link="ss://$(echo -n "${ssciphers}:${sspass}@${ip}:${ssport}" | base64 -w 0)#ss_${ip}"
	local link="https://${magic}.github.io/tools/qr.html#${ss_link}"
	echo
